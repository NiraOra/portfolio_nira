				...
				>
					<motion.div variants={itemVariants} className="space-y-4">
						...
						<p className="text-body text-muted-foreground max-w-xl">
							Research conducted using the Human Phenotype Project (HPP) under the
							supervision of Professor Imran Razzak, focusing on AI applications in
							healthcare and genomics. (Note: only preprints available currently)
						</p>
					</motion.div>
					...
					<motion.div variants={itemVariants} className="space-y-6">
						{publications.map((pub, index) => (
							<motion.article
								key={pub.title}
								id={`publication-${index}`} // Add unique id for each publication
							...
							>
								<div className="flex items-start justify-between gap-4">
									<div className="flex-1 space-y-3">
										...
										<p className="text-small">{pub.authors}</p>
